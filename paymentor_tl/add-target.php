...
$sid=$_SESSION['sid'];
$sql=mysqli_query($conn,"select name from add_paymentor where tl_name='$sid'");
$name1=array();
while($row=mysqli_fetch_array($sql,MYSQLI_ASSOC))
    ...
    if(isset($_POST['sub']))
    {
       ...
       $sql_update=mysqli_query($conn,"update add_varifier set month_target_cmplt='NULL',target_cmplt='NULL',today_target='NULL',today_target_left='NULL',target_add_month='NOW()'");
    	
        if(!empty($target))
        {
        	$sql1=mysqli_query($conn,"update  add_paymentor set target='$target1' where tl_name='$sid' AND name='$documentor1'")or trigger_error(E_USER_ERROR);
        	if($sql1!=false)
        	{ ?>
