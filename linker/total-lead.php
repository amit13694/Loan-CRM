...
$sql4=mysqli_query($conn,"select target from add_linker where email='$main_tl_name'");
$target1=mysqli_fetch_array($sql4);
...
$sql7=mysqli_query($conn,"select today_target from add_linker where email='$main_tl_name'");
$target_today=mysqli_fetch_array($sql7);
$limit_target=($target_today['today_target']-$dashboard_target_cmplt['target_cmplt']);
$sql=mysqli_query($conn,"select * from documentor_document where varifier_status='yes' AND linkup_cnfrm='' order by id DESC LIMIT $limit_target");
$sql8=mysqli_query($conn,"select tl_name from add_linker where email='$main_tl_name'");
...
while($row=mysqli_fetch_array($sql,MYSQLI_ASSOC))
{
  ...
  $linkup_status[]=$row['linkup_cnfrm'];
  $id[]=$row['id'];
  $name[]=$row['name'];
$address[]=$row['address'];
...
//-------------------------------------------------
$fetching_monthwise_target_left=mysqli_query($conn,"select DAY(target_add_month) AS t, MONTH(target_add_month) AS p from add_linker where email='$main_tl_name'");
...
$fetch_data=mysqli_query($conn,"select target_cmplt,month_target_cmplt,target,today_target from add_linker where email='$main_tl_name'");
                      ...
                      Congratulation!! Your Traget Completed
                                ...
                                <?php
                                $add_benefit=mysqli_query($conn,"update add_linker set day_$day='1' where email='$main_tl_name' ");
...
{           
          ...
          $_SESSION['j'] = isset($_SESSION['j']) ? ++$fetch_target_cmplt['month_target_cmplt'] : ++$fetch_target_cmplt['month_target_cmplt'];
                              ...
                              $sql_insert_target_cmplt=mysqli_query($conn,"update add_linker set target_cmplt='$target_in_database',month_target_cmplt='$data' where email='$main_tl_name'");
          }
    ...
    $user_filename=$empname1.".".$ext_user;
          //----------------------------------------------
          $sql1=mysqli_query($conn,"update documentor_document set name='$empname1',address='$empadd1',zip_code='$empzip1',contact='$empnumb1',email='$empemail1',adhar_number='$empuid1',pan_number='$emppan1',pan_image='$pan_filename',adhar_image='$uid_filename',bank_image='$bank_filename',photo='$user_filename',other_document='$other_filename',dob='$empdob1',qualification='$empqualification1',civil_report='$empcivil1',follow_up='$empfollow1',linkup_cnfrm='$empcomment1',linker_name='$main_tl_name',tl_name='$tl_name',linkup_dat=NOW(),follow_cmnt='$follow_cmnt1' where id='$emp_id1'");
          if($sql1!=false)
          { 
...
<?php                
                              ...
                              // $target_in_database=$_SESSION['i'];
                              // $sql_insert_target_cmplt=mysqli_query($conn,"update add_linker set target_cmplt='$target_in_database' where email='$main_tl_name'");
                              // if($target_in_database==$target1['target'])
                              // {
                              //   $_SESSION['i']=0;
                              // }
 	   		...
 	   		<tbody>
                  ...
                  for($i=0;$i<count($name);$i++)
                  {
                    ...
                    <tr class="text-light" style="background:#6D6E6A;" >
                      ...
                      <td><?php echo $linkup_status[$i];?></td>
        ...
        <div class="form-group">
          ...
          <input type="number" name="emppan" class="form-control" id="emppan">
               ...
               <input type="hidden" name="emp_id" id="emp_id">
                <div class="form-group">
                    <label for="comment">Linkup Status</label>
    ...
    $(document).on('click' ,'.data_edit' , function(){
       ...
       $.ajax({
        ...
        success:function(data)
        {
          ...
          $('#empcomment').val(data.linkup_cnfrm);
