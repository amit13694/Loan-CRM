...
$sql=mysqli_query($conn,"select today_target from add_varifier where email='$sid'");
$target1=mysqli_fetch_array($sql);
$limit_target=($target1['today_target']-$dashboard_target_cmplt['target_cmplt']);
$sql=mysqli_query($conn,"select * from documentor_document where varifier_status='' AND varifier_flw='vflw' AND MONTH(varified_dat)=MONTH(CURDATE()) order by id DESC");
...
//-------------------------------------------------
$fetching_monthwise_target_left=mysqli_query($conn,"select MONTH(target_add_month) AS t, YEAR(target_add_month) AS p from add_varifier where email='$sid'");
$fetch_left_data=mysqli_fetch_array($fetching_monthwise_target_left);
$month=$fetch_left_data['t'];
$year=$fetch_left_data['p'];
//-------------------------------------------------
if($year!=date('Y'))
{
  $update_all_data=mysqli_query($conn,"update add_varifier set day_1='0',day_2='0',day_3='0',day_4='0',day_5='0',day_6='0',day_7='0',day_8='0',day_9='0',day_10='0',day_11='0',day_12='0'");
}
//-------------------------------------------------
$fetch_data=mysqli_query($conn,"select target_cmplt,month_target_cmplt,target from add_varifier where email='$sid'");
                                $fetch_target_cmplt=mysqli_fetch_array($fetch_data);
                                 if($fetch_target_cmplt['target_cmplt']==$target1['today_target'])
                              {
                                 $sql_insert_target_cmplt=mysqli_query($conn,"update add_varifier set target_cmplt='0' where email='$sid'");
                              }
                              if($fetch_target_cmplt['target']==$fetch_target_cmplt['month_target_cmplt'])
                              { 
                             ?>
                      ...
                      Congratulation!! Your Traget Completed
                                ...
                                <?php
                                $add_benefit=mysqli_query($conn,"update add_varifier set day_$month='1' ");
                                $update_month_target=mysqli_query($conn,"update add_varifier set month_target_cmplt='0' ");
                              }
if(isset($_POST['sub']))
{
         @$empname1=$_POST['empname'];
          ...
          $_SESSION['j'] = isset($_SESSION['j']) ? ++$fetch_target_cmplt['month_target_cmplt'] : $fetch_target_cmplt['month_target_cmplt'];
                              ...
                              $sql_insert_target_cmplt=mysqli_query($conn,"update add_varifier set target_cmplt='$target_in_database',month_target_cmplt='$data' where email='$sid'");
          }
  ...
  //---------------------------------------------------------------------------------------------------
          $sql1=mysqli_query($conn,"update documentor_document set name='$empname1',address='$empadd1',zip_code='$empzip1',contact='$empnumb1',email='$empemail1',adhar_number='$empuid1',pan_number='$emppan1',pan_image='$pan_filename',adhar_image='$uid_filename',bank_image='$bank_filename',photo='$user_filename',other_document='$other_filename',dob='$empdob1',qualification='$empqualification1',civil_report='$empcivil1',follow_up='$empfollow1',varifier_status='$empvar1',varifier_name='$sid',tl_name='$tl_name',varified_dat=NOW(),follow_cmnt='$follow_cmnt1',varifier_flw='$varifier_flw1' where id='$emp_id1'");
          if($sql1!=false)
          {
...
<?php 
                     header("Refresh:1; url=varified-follow.php");
          }
          else
          { ?>
             <div class="alert alert-light alert-dismissible" style="width:30%; margin-left: 44%;">
...
if(isset($_POST['delete_sub']))
{
  ...
  if($sql4!=false)
  {
    header("location:varified-follow.php");
