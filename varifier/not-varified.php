...
$sid=$_SESSION['sid'];
$sql=mysqli_query($conn,"select * from documentor_document where varifier_status='no' AND varifier_name='$sid' AND MONTH(varified_dat)=MONTH(CURDATE()) order by id DESC");
...
while($row=mysqli_fetch_array($sql,MYSQLI_ASSOC))
{
  ...
  $var_status[]=$row['varifier_status'];
  $id[]=$row['id'];
  $name[]=$row['name'];
$address[]=$row['address'];
...
<?php 
                     header("Refresh:1; url=not-varified.php");
          }
          else
          { ?>
             <div class="alert alert-light alert-dismissible" style="width:30%; margin-left: 44%;">
...
if(isset($_POST['delete_sub']))
{
  ...
  if($sql4!=false)
  {
    header("location:not-varified.php");
        ...
        <div class="form-group">
          ...
          <input type="number" name="emppan" class="form-control" id="emppan">
               ...
               <div class="form-group">
                    ...
                    <input type="text" name="var" class="form-control" id="empvar">
               </div>
  ...
  $('#myTable').DataTable({
    ...
    $(document).on('click' ,'.data_edit' , function(){
       ...
       $.ajax({
        ...
        {
            $('#empvar').val(data.varifier_status);
            $('#emp_id').val(data.id);
