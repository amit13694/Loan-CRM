...
$sql=mysqli_query($conn,"select password from admin where usename='$admin_name' AND status='linker_tl'");
...
if(isset($_POST['sub']))
{
	...
	{ 
		...
		{
			$sql=mysqli_query($conn,"update admin set password='$new' where usename='$admin_name' AND status='linker_tl'");
      $sql1=mysqli_query($conn,"update add_tl set tl_pass='$new' where email='$admin_name' AND status='linker_tl'");
