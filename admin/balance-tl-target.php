...
$sid=$_SESSION['sid'];
$sql=mysqli_query($conn,"select name,email from add_tl where status='paymentor_tl'");
$name1=array();
$email1=array();
//automatic set target of tl if admin not set at every month target become same as previuos month
$sql_auto=mysqli_query($conn,"select MONTH(target_add_month) AS m,target from add_tl where status='paymentor_tl'");
...
{
  $sql_update=mysqli_query($conn,"update add_tl set target_add_month='NOW()',target_cmplt='NULL',target_left='NULL',target='$auto_update_target',daily_target='$auto_update_daily_target',target_left='NULL',daily_target_cmplt='NULL',daily_target_left='NULL' where status='paymentor_tl'");
       $sql_update1=mysqli_query($conn,"update add_paymentor set month_target_cmplt='NULL',target_cmplt='NULL',today_target='NULL',today_target_left='NULL',target_add_month='NOW()'");
...
while($row=mysqli_fetch_array($sql,MYSQLI_ASSOC))
  ...
  $sql_update=mysqli_query($conn,"update add_tl set target_add_month='NOW()',target_cmplt='NULL',target_left='NULL',daily_target_cmplt='NULL',daily_target_left='NULL' where email='$documentor1'");
       $sql_update1=mysqli_query($conn,"update add_paymentor set month_target_cmplt='NULL',target_cmplt='NULL',today_target='NULL',today_target_left='NULL',target_add_month='NOW()',tl_name='$documentor1'");
    	
        if(!empty($target))
        {
        	$sql1=mysqli_query($conn,"update  add_tl set target='$target1',daily_target='$tl_daily_target' where status='paymentor_tl' AND email='$documentor1'")or trigger_error(E_USER_ERROR);
        	if($sql1!=false)
        	{ ?>
