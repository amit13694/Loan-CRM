...
$sql_tl=mysqli_query($conn,"select add_tl.name,add_tl.email from add_tl INNER JOIN admin ON add_tl.email=admin.usename where add_tl.status='documentor_tl'");
...
if(isset($_POST['sub']))
{
	...
	{
		$sql=mysqli_query($conn,"insert into admin(usename,name,password,status,tl_name,documentor_unique,logout,sidenav,container,header)values('$empemail','$empname','$emppass','documentor','$tl1','$unique',NOW(),'#343A40','#6D6E6A','#343A40')");
		$sql1=mysqli_query($conn,"insert into add_documentor(name,contact,email,joining_date,docu_password,tl_name,unique_number,target)values('$empname','$empnumber','$empemail','$empdate','$emppass','$tl1','$unique','0')")or trigger_error("sql error".mysqli_error($conn),E_USER_ERROR);
