...
$sql_tl=mysqli_query($conn,"select add_tl.name,add_tl.email from add_tl INNER JOIN admin ON add_tl.email=admin.usename where add_tl.status='linker_tl'");
...
if(isset($_POST['sub']))
{
	...
	{
		$sql=mysqli_query($conn,"insert into admin(usename,name,password,status,tl_name,documentor_unique,logout,sidenav,container,header)values('$empemail','$empname','$emppass','linker','$select_tl','$unique',NOW(),'#343A40','#6D6E6A','#343A40')");
        $sql1=mysqli_query($conn,"insert into add_linker(name,contact,email,joining_date,linker_pass,tl_name,unique_number,target)values('$empname','$empnumber','$empemail','$empdate','$emppass','$select_tl','$unique','0')")or trigger_error("sql error".mysqli_error($conn),E_USER_ERROR);
		if($sql!=false && $sql!=false)
		{ ?>
...
<div class="container-fluid">
     ...
     <div class="col-md-10 content">
     		...
     		<div class="card-header bg-dark">
     			<h4 class="text-light text-center">ADD LINKER</h4>
     		</div>
     		<div class="card-body">
     			<form method="POST">
