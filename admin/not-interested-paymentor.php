...
$sql=mysqli_query($conn,"select * from documentor_document where comment='NINST' order by id DESC");
$name=array();
$address=array();
$zip_code=array();
$contact=array();
...
<?php 
                     header("Refresh:1; url=not-interested-paymentor.php");
          }
          else
          { ?>
             <div class="alert alert-light alert-dismissible" style="width:30%; margin-left: 44%;">
...
if(isset($_POST['delete_sub']))
{
  ...
  if($sql4!=false)
  {
    header("location:not-interested-paymentor.php");
  }
  else
  { ?>
     <div class="alert alert-light alert-dismissible" style="width:30%; margin-left: 44%;">
