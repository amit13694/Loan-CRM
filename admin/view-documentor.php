...
$sql=mysqli_query($conn,"select add_documentor.id AS doc_id,add_documentor.name AS doc,admin.login,admin.logout,add_documentor.contact,add_documentor.email,add_documentor.joining_date,add_documentor.docu_password,add_tl.name,add_documentor.target,add_documentor.target_cmplt,admin.attendance from add_documentor INNER JOIN admin ON admin.usename=add_documentor.email INNER JOIN add_tl on add_tl.email=add_documentor.tl_name");
...
if(isset($_POST['sub']))
{
  $sql2=mysqli_query($conn,"update admin set password='$emppass1' where usename='$empemail1' AND documentor_unique='$unique_id1'");
$sql1=mysqli_query($conn,"update add_documentor set name='$empname1',contact='$empnumber1',email='$empemail1',joining_date='$empdate1',docu_password='$emppass1',target='$emptarget1' where email='$empemail1' AND unique_number='$unique_id1'");
if($sql1!=false)
{ ?>
...
<?php 
                     header("Refresh:1; url=view-documentor.php");
}
else{ ?>
                  <div class="alert alert-light alert-dismissible" style="width:30%; margin-left: 44%;">
                      <button type="button" class="close" data-dismiss="alert">&times;</button>
    ...
    $(document).on('click' ,'.data_edit' , function(){
       ...
       $.ajax({
        ...
        success:function(data)
        {
          ...
          $('#emppass').val(data.docu_password);
          $('#emp_id').val(data.id);
