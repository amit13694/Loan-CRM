...
$sql=mysqli_query($conn,"select add_linker.id AS link_id,add_linker.name AS link,admin.login,admin.logout,add_linker.contact,add_linker.email,add_linker.joining_date,add_linker.linker_pass,add_tl.name,add_linker.target,add_linker.target_cmplt,admin.attendance from add_linker INNER JOIN admin ON admin.usename=add_linker.email INNER JOIN add_tl on add_tl.email=add_linker.tl_name");
...
if(isset($_POST['sub']))
{
  $sql2=mysqli_query($conn,"update admin set password='$emppass1' where usename='$empemail1' AND linker_unique='$unique_id1'");
$sql1=mysqli_query($conn,"update add_linker set name='$empname1',contact='$empnumber1',email='$empemail1',joining_date='$empdate1',linker_pass='$emppass1',target='$emptarget1' where email='$empemail1' AND unique_number='$unique_id1' ");
if($sql1!=false)
{ ?>
...
<?php 
                     header("Refresh:1; url=view-linker.php");
}
else{ ?>
                  <div class="alert alert-light alert-dismissible" style="width:30%; margin-left: 44%;">
                      <button type="button" class="close" data-dismiss="alert">&times;</button>
...
if(isset($_POST['delete_sub']))
{
  ...
  {
    header("location:view-linker.php");
  }
  else
  { ?>
     <div class="alert alert-light alert-dismissible" style="width:30%; margin-left: 44%;">
