...
$sql=mysqli_query($conn,"select password from admin where usename='$admin_name' AND status='payment'");
...
if(isset($_POST['sub']))
{
	...
	{ 
		...
		{
			$sql=mysqli_query($conn,"update admin set password='$new' where usename='$admin_name' AND status='payment'");
      $sql1=mysqli_query($conn,"update add_paymentor set paymentor_pass='$new' where email='$admin_name'");
